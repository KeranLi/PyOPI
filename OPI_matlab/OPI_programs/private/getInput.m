...
if ~isempty(sampleFile)
    ...
    %c = char(strip(X1)); found a bug here by Keran Li, 0310, 2026
    % 确保 X1 是文本（兼容数字和空值）
    if ~ischar(X1) && ~isstring(X1)
        X1 = string(X1);
    end
    c = char(strip(X1));
    ...
    % 处理数值列（兼容数字和文本格式）
    % Excel列顺序：1=Longitude, 2=Latitude, 3=Elevation, 4=d2H, 5=d18O, 6=Type
    sampleLon = str2double(string(X(:,1)));      % 第1列：经度
    sampleLat = str2double(string(X(:,2)));      % 第2列：纬度
    sampleElev = str2double(string(X(:,3)));     % 第3列：海拔（需要读）
    sampleD2H = str2double(string(X(:,4)))*1e-3; % 第4列：d2H（原来是第3列，改为第4列）
    sampleD18O = str2double(string(X(:,5)))*1e-3;% 第5列：d18O（原来是第4列，改为第5列）
    
    % 处理文本列（Type）
    sampleLC = char(upper(string(X(:,6))));      % 第6列：Type（原来是第5列，改为第6列）
    sampleLC = sampleLC(:,1);
