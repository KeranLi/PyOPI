function [chiR2, nu, stdResiduals, ...
    ...
    ~, sampleD18O, cov, nParametersFree, isFit)
% calc_TwoWinds_OxygenOnly provides a full calculation of misfit, climate
% and isotope values for a specified solution vector, beta, for two
% moisture sources (TwoWind), using d18O only for the objective function.
...
% Oxygen-only objective variant added for corrected d18O constraints.
...
d18O0_1 = beta(7);      % d18O of base precipitation (per unit)
dD18O0_dLat_1 = beta(8);% latitudinal gradient of base-prec d18O (1/deg lat)
d2H0_1 = bMWLSample(1) + bMWLSample(2)*d18O0_1;
dD2H0dLat_1 = bMWLSample(2)*dD18O0_dLat_1;
...
d18O0_2 = beta(17);     % d18O of base precipitation (per unit)
dD18O0_dLat_2 = beta(18);% latitudinal gradient of base-prec d18O (1/deg lat)
d2H0_2 = bMWLSample(1) + bMWLSample(2)*d18O0_2;
dD2H0dLat_2 = bMWLSample(2)*dD18O0_dLat_2;
...
%... d18O base composition is fit directly in oxygen-only mode.
...
if nu > 0
    %... Calculate chiR2 using d18O only for wet sample locations.
    minSigma18O = 1e-3; % 1 per mil, used when formula-derived d2H makes cov degenerate.
    sigma18O = max(sqrt(cov(2,2)), minSigma18O);
    r = sampleD18O(iWet) - d18OPred(iWet);
    chiR2 = tinv(0.84134, nu)*sum((r./sigma18O).^2)/nu;
    %... Calculate standardized d18O residuals for all samples.
    stdResiduals = abs(sampleD18O - d18OPred)./sigma18O;
else
    ...
    if isFit
        chiR2 = realmax('double');
        nu = nSamples - nParametersFree;
        stdResiduals = nan(nSamples,1);
        return
    end
    rethrow(ME)
