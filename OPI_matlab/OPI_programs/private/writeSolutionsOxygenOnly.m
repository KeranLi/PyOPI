function writeSolutionsOxygenOnly(data, varargin)
...
switch true
    case strcmp(data, 'initialize')
        ...
        logFilename = [runPath, '/', 'opiFit_TwoWinds_OxygenOnly_Solutions.txt'];
        if isfile(logFilename)
            movefile(logFilename, [runPath, '/', 'opiFit_TwoWinds_OxygenOnly_Solutions.bk!']);
        end
        ...
        fid = fopen(logFilename, 'w', 'native', 'UTF-8');
        fprintf(fid, '%% Solution file for optimization using opiFit_TwoWinds_OxygenOnly\n');
        fprintf(fid, '%% Objective: d18O residuals only; d2H is derived only for compatibility outputs.\n');
