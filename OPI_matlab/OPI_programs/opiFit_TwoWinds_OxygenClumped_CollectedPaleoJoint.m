function opiFit_TwoWinds_OxygenClumped_CollectedPaleoJoint(runFile)
...
externalFile = fullfile(runPath, 'collected_proxy_observations.csv');
if ~isfile(externalFile)
    externalFile = fullfile(dataPath, 'collected_proxy_observations.csv');
end
if isfile(externalFile)
...
isFit = true;
beta = fminCRS3(@(beta) ...
    calc_TwoWinds_OxygenClumpedObjective_CollectedPaleoJoint(beta, fC, hR, ...
    x, y, lat, lat0, hGrid, bMWLSample, ijCatch, ptrCatch, ...
    sampleD2H, sampleD18O, cov, nParametersFree, isFit, ...
    clumped, sampleLon, sampleLat, dolomiteOffsetC, sigmaOffsetC, clumpedSeason, external), ...
    lB, uB, mu, epsilon0, isParallel, @writeSolutionsOxygenOnly, solutions);

[chiR2Total, nuTotal, detail] = calc_TwoWinds_OxygenClumpedObjective_CollectedPaleoJoint(beta, fC, hR, ...
    x, y, lat, lat0, hGrid, bMWLSample, ijCatch, ptrCatch, ...
...
fprintf('external proxy records used: %d\n', detail.nExternal)
