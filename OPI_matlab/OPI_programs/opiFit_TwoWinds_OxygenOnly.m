function opiFit_TwoWinds_OxygenOnly(runFile)
% opiFit_TwoWinds_OxygenOnly finds a best-fit opi solution for isotope
% data using d18O only in the objective function.
%
% v. 3.6 July, 2021
...
fprintf('Objective: oxygen-only; reduced chi-square uses d18O residuals only.\n')
...
fprintf('d18O for base precipitation: %g, %g per mil\n', [lB(7), uB(7)]*1e3)
fprintf('d18O latitude gradient for base precipitation: %g, %g  per mil/deg lat\n', [lB(8), uB(8)]*1e3)
...
beta = fminCRS3(@(beta) ...
    ...
    lB, uB, mu, epsilon0, ...
    isParallel, @writeSolutionsOxygenOnly, solutions);
...
dD2H0_dLat_1 = bMWLSample(2)*dD18O0_dLat_1;
...
dD2H0_dLat_2 = bMWLSample(2)*dD18O0_dLat_2;
...
fprintf('Derived d2H for compatibility: %.1f per mil\n', d2H0_1*1e3)
fprintf('Derived d2H latitude gradient: %.3f per mil/deg lat\n', dD2H0_dLat_1*1e3)
...
fprintf('\n------------------ Quality of Fit -------------------\n')
fprintf('Objective used for fit: d18O only\n')
