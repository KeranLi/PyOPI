...
def calc_two_winds(beta, f_c, h_r, x, y, lat, lat0, h_grid, b_mwl_sample,
    ...
    # Calculate isotope grids (returns tuple)
    ...
    # Get state 2 grids (may have different size than state 1)
    ...
    interpolator = RegularGridInterpolator((s_2, t_2), f_m_wind_2, method='linear',
                                           bounds_error=False, fill_value=None)
    f_m_grid_2 = np.reshape(interpolator((s_xy_2.flatten(), t_xy_2.flatten())), s_xy_2.shape)
    del f_m_wind_2
    ...
    if not is_fit:
        if r_h_wind_2.size > 1:
            interpolator = RegularGridInterpolator((s_2, t_2), r_h_wind_2, method='linear',
                                                   bounds_error=False, fill_value=None)
            r_h_grid_2 = np.reshape(interpolator((s_xy_2.flatten(), t_xy_2.flatten())), s_xy_2.shape)
            del r_h_wind_2
        else:
